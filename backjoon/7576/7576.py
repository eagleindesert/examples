...
def main():
    ...
    for pos in start_pos:
        pending.append(deque([pos]))
        tomato_day[pos[0]][pos[1]] = 0
        tomato_visited[pos[0]][pos[1]] = 1

    pending_start_pos = list(range(len(start_pos)))
    while pending_start_pos:
        # 한 층 반복
        for pnt in pending_start_pos:
            for _ in range(len(pending[pnt])):
                cur = pending[pnt].popleft()
                ...
                for node in next:
                    ...
                    pending[pnt].append(node)
                    tomato_visited[node[0]][node[1]] = 1
                    tomato_day[node[0]][node[1]] = tomato_day[cur[0]][cur[1]] + 1

        pending_start_pos.clear()
        for idx, deq in enumerate(pending):
            if deq:
                pending_start_pos.append(idx)
